...
First of all, there are 6 layers (`GPHASE_LAYER`) that represent game states :
...
Those layers regroup a collection of `GPHASEs` that are related to game functions in their name.
...
A GPHASE is represented by a `GPHASE_ID`, an enum list of all possible game phases, here are the first few phases of the game:
...
Each `GPHASE` has 4 methods: `ini_func`, `pre_func`, `after_func` and `end_func`.

`ini_func` and `pre_func` have no arguments while `after_func` and `end_func` take a `GPHASE_ID` as an argument.
...
Take note that the `SUPER GPHASE` is at **position 0** just like its enum.

The following list is all of the game's GPhases and their related data.
